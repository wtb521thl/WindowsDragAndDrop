...
{
    public class OutLineRightUp : OutLine
    {
        ...
        protected override void DragLine()
        {
            base.DragLine();
            //newObjDisX = startDragObjSizeDeltaX + (Input.mousePosition.x - startDragMousePos.x);
            selfRect.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, startDragObjPosX / (selfRect.parent.GetRectTransform().GetSize().x / 1920f), newObjDisX);
            ...
            if (limitSize.x > selfRect.GetLocalSize().x + newObjDisX)
            {
                selfRect.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Left, startDragObjPosX / (selfRect.parent.GetRectTransform().GetSize().x / 1920f), limitSize.x);
            }

            if (limitSize.y > selfRect.GetLocalSize().y + newObjDisY)
            {
                selfRect.SetInsetAndSizeFromParentEdge(RectTransform.Edge.Bottom, startDragObjPosY / (selfRect.parent.GetRectTransform().GetSize().y / 1080f), limitSize.y);
            }
        }
    }
